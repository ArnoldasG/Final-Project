...
function adsDisplay(firebaseData, favoritesData, userID) {
  ...
  for (let data in firebaseData) {
    ...
    let isOwnerOrAdmin =
      userID === firebaseData[data].userID ||
            ...
            <div class="card photo-card" data-id=${data} style="width: 18rem;">
                <div class="photo-container">
                    <img class="card-img-top" src=${
                      firebaseData[data].picture
                    } alt=${firebaseData[data].name}>
                    <div class="favorite-logo-container">
                        <i class="${
                          isFavorited ? "fa-solid" : "fa-regular"
                        } fa-star favoriteBtn" data-id=${data}></i>
                    </div>
                    <div class="fa-solid-container">
                        <i class="fa-solid fa-square-minus adsDelI ${
                          isOwnerOrAdmin ? "deleteAdsBtn" : ""
                        }"></i>
                        <i class="fa-solid fa-square-pen adsUpdateI ${
                          isOwnerOrAdmin ? "updateAdsBtn" : ""
                        }"></i>
                        <i class="fa-solid fa-comment-dots commentBtn"></i>
                    </div>
                </div>
                <div class="card-body">
                    <h3 class="card-title photo-title">${
                      firebaseData[data].name
                    }</h3>
...
function favoriteBtnsFunctionality(userID) {
  ...
  favoriteBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      get(ref(database, "favorites/" + adsUniqueID + userID)).then(
        (snapshot) => {
          if (snapshot.exists()) {
...
function searchBtnFunctionality(firebaseData, userID) {
  ...
  searchInfoContainer.innerHTML = `<input type="text" class="form-control" id="searchInput" placeholder="search by the name">
      ...
      get(ref(database, `ads/${uniqueAdsBtnID}`)).then((snapshot) => {
        if (snapshot.exists()) {
          remove(ref(database, `ads/${uniqueAdsBtnID}`))
            .then(() => {
              universalModalFunctionality(
                "Your advertisement deleted successfully!"
              );
              window.location.reload();
            })
            .catch((error) => {
      ...
      get(ref(database, `ads/${uniqueAdsBtnID}`)).then((snapshot) => {
        ...
        nextUploadAdsBtn.addEventListener("click", (e) => {
          ...
          update(ref(database, `ads/${uniqueAdsBtnID}`), {
            ...
            .then(() => {
              universalModalFunctionality(
                "Your advertisement updated succsessfully!"
              );
              window.location.reload();
            })
            .catch((error) => {
...
function modalCommentsRender(userID, uniquecommentBtnID) {
  ...
  get(ref(database, `comments/`)).then((snapshot) => {
    const commentsData = snapshot.val();
    get(ref(database, "Users/" + userID)).then((userSnapshot) => {
      const userData = userSnapshot.val();
      get(ref(database, "ads/" + uniquecommentBtnID)).then(
        (adsUserSnapshot) => {
                ...
                commentsTbody.innerHTML += `
                                <tr data-id=${c}>
                                    ...
                                    <td class="text-center" >${
                                      emailExistOrNo
                                        ? commentsData[c].userEmail
                                        : "Deleted user"
                                    }</td>
                                    <td class="text-center" >${
                                      commentsData[c].comment
                                    }</td>
