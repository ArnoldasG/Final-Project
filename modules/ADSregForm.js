...
function creatingAdsForm() {
  ...
  mainAdsFormContainer.innerHTML = `
        <div class="row d-flex justify-content-center ads-reg-form">
            <div class="col-md-8">
                <div class="card">
                    <h2 class="card-title text-center">Įkelkite savo skelbimą</h2>
                    <div class="card-body py-md-4">
                        <form>
                            <div class="form-group">
                                <input type="text" class="form-control" id="ads-name" placeholder="Skelbimo pavadinimas">
                            </div>
                            <div class="form-group">
                                <select class="form-select form-control" id="form-selection" aria-label="Default select example">
                                    <option value="Select your category" disabled selected>Pasirinkite kategoriją</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <textarea class="form-control" id="ads-about" rows="5"
                                placeholder="Apibūdinkite savo skelbimą"></textarea>
                            </div>
                            <div class="form-group">
                                <input type="number" class="form-control" id="price" placeholder="Kaina">
                            </div>
                            <div class="my-3">
                                <input type="text" class="form-control" id="picture-name" placeholder="Įterpkite skelbimo nuotraukos nuorodą">
                            </div>
                            <div class="d-flex flex-row align-items-center justify-content-between">
                                <button class="btn btn-primary" id="uploadAdsBtn">Įkelti</button>
  ...
  uploadAdsBtn.addEventListener("click", (e) => {
    ...
    if (adsNameInput.value.length < 3) {
      universalModalFunctionality(
        "Advertisement name should be more than 3 symbols"
      );
